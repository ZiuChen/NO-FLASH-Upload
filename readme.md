...
| ----------- | --------------------- | --------- | ------ |
| **Chrome**  | `Tampermonkey 4.16.1` | `2.2.14` | ✔      |
| **Edge**    | `Tampermonkey 4.16.1` | `2.2.14` | ✔      |
| **FireFox** | `Tampermonkey 4.16.1` | `2.2.14` | ✔      |
...
* `src`：源文件目录
  ...
  * `style`：存放样式文件的目录
  * `ts`：存放逻辑与网络请求的`Typescript`脚本的目录
  * `index.ts`：脚本的入口文件
  
  **注意：** 目前项目有两个分支：旧的`script`分支与新的`Vue`分支，分别对应`1.x`版本与`2.x`版本，`1.x`版本由于课程平台更新，部分功能已基本不可用，将不再维护。
